"""メルカリカテゴリDBを使ってHTS章番号を日本語テキストから推定するモジュール。

mercari_categories.json の jp_name を入力テキストで部分マッチして
対応する HTS 章番号と英語キーワードを返す。
"""
import json
from pathlib import Path

_DB_PATH = Path(__file__).parent / "data" / "mercari_categories.json"
...
def lookup_chapters(text: str) -> list[tuple[str, list[str]]]:
    """日本語テキストにメルカリカテゴリ名が含まれていれば
    対応する (hts_chapter, en_keywords) のリストを返す。
    jp_name が長い(=詳細な)エントリを優先する。
    同じ章は最初の1件のみ返す。
    """
    if not text:
        return []
