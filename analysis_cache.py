...
_REPO_DB = Path(__file__).parent / "data" / "analysis_cache.db"


def _resolve_db_path() -> Path:
    """書き込み可能なDBパスを返す。
    リポジトリ配下が書き込み可ならそれを使い（ローカル）、読み取り専用環境
    （Streamlit Cloud等）では一時ディレクトリへ退避し、コミット済みDBをシードする。
    """
    repo_dir = _REPO_DB.parent
    try:
        repo_dir.mkdir(exist_ok=True)
        test = repo_dir / ".write_test"
        test.write_text("x", encoding="utf-8")
        test.unlink()
        return _REPO_DB
    except Exception:
        tmp = Path(tempfile.gettempdir()) / "hts_analysis_cache.db"
        if not tmp.exists() and _REPO_DB.exists():
            try:
                shutil.copy2(_REPO_DB, tmp)
            except Exception:
                pass
        return tmp
...
def _get_conn() -> sqlite3.Connection:
    if not getattr(_local, "conn", None):
        ...
        # 並行書き込みの「database is locked」を緩和
        try:
...
def save_analysis(image_bytes: bytes, analysis: dict) -> None:
    """L1: 画像解析結果を保存する。書き込み失敗は無視（判定は継続）。"""
    h = _image_hash(image_bytes)
...
def save_hts(analysis: dict, results) -> None:
    """L2: HTS分類結果を分析フィンガープリントで保存する。書き込み失敗は無視。
